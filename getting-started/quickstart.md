...
- A registered account on a website that supports the Servio protocol — you do not need to own the site, just have user-level access
...
New accounts receive **1,850 welcome credits** to get started at no cost.
...
::: tip
You do not have to add a site through the Vault first. You can paste the connection prompt generated by a Servio-enabled site directly into the chat window — the platform will recognize it and offer to confirm the connection on the spot.
:::
...
2. Click **New Session**.
3. Select an AI model (e.g., Claude Sonnet 4.6).
